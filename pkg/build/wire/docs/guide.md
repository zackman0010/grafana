...
an interface. Wire matches inputs to outputs via [type identity](https://golang.org/ref/spec#Type_identity), so the
...
[return concrete types](https://github.com/golang/go/wiki/CodeReviewComments#interfaces). Instead, you can declare an interface binding in a
provider set:
