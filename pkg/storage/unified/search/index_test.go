...
func TestIndexDashboard(t *testing.T) {
	...
	list := &resource.ListResponse{Items: []*resource.ResourceWrapper{{Value: data}}}
...
func TestSearchFolder(t *testing.T) {
	...
	list := &resource.ListResponse{Items: []*resource.ResourceWrapper{{Value: dashboard}, {Value: folder}}}
	index := newTestIndex(t, 1)
...
func TestSearchDashboardsAndFoldersOnly(t *testing.T) {
	...
	list := &resource.ListResponse{Items: []*resource.ResourceWrapper{{Value: dashboard}, {Value: folder}, {Value: playlist}}}
	index := newTestIndex(t, 1)
...
func TestLookupNames(t *testing.T) {
	...
	list := &resource.ListResponse{Items: []*resource.ResourceWrapper{}}
	for _, f := range folders {
		list.Items = append(list.Items, &resource.ResourceWrapper{Value: []byte(f)})
	}
	index := newTestIndex(t, 1)
...
func TestIndexDashboardWithTags(t *testing.T) {
	...
	list := &resource.ListResponse{Items: []*resource.ResourceWrapper{{Value: data}, {Value: data2}}}
...
func TestIndexBatch(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		list := &resource.ListResponse{Items: loadTestItems(strconv.Itoa(i), ns)}
		start := time.Now()
...
func loadTestItems(uid string, tenants []string) []*resource.ResourceWrapper {
	example := `{
		"kind": "<kind>",
		"title": "test",
		"metadata": {
	...
	items := []*resource.ResourceWrapper{}
	for i := 0; i < 10000; i++ {
		res := strings.Replace(example, "<uid>", strconv.Itoa(i)+uid, 1)
		// shuffle kinds
		...
		items = append(items, &resource.ResourceWrapper{Value: []byte(res)})
	}
	return items
}
...
func assertSearchCountEquals(t *testing.T, index *Index, search string, kind []string, expected int64) {
	req := &resource.SearchRequest{Query: search, Tenant: testTenant, Limit: expected + 1, Offset: 0, Kind: kind}
	start := time.Now()
...
func assertSearchGroupCountEquals(t *testing.T, index *Index, search string, group string, expected int64) {
	groupBy := []*resource.GroupBy{{Name: group, Limit: 100}}
	req := &resource.SearchRequest{Query: search, Tenant: testTenant, Limit: expected + 1, Offset: 0, GroupBy: groupBy}
	results, err := index.Search(testContext, req)
	require.NoError(t, err)
