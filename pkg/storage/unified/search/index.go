...
func NewIndex(s resource.ResourceServer, opts Opts, tracer tracing.Tracer) *Index {
	idx := &Index{
		s:      s,
		opts:   opts,
...
func (i *Index) AddToBatches(ctx context.Context, list *resource.ListResponse) ([]string, error) {
...
func (i *Index) Init(ctx context.Context) error {
	...
	for _, rt := range resourceTypes {
		...
		r := &resource.ListRequest{Options: rt, Limit: int64(i.opts.ListLimit)}
...
func (i *Index) writeBatch(ctx context.Context, list *resource.ListResponse) error {
...
func (i *Index) Delete(ctx context.Context, uid string, key *resource.ResourceKey) error {
...
func (i *Index) Search(ctx context.Context, request *resource.SearchRequest) (*IndexResults, error) {
	...
	for _, group := range request.GroupBy {
		...
		for _, term := range groupByFacet.Terms.Terms() {
			groups = append(groups, &resource.Group{Name: term.Term, Count: int64(term.Count)})
		}
	}
...
// TODO - fetch from api
func fetchResourceTypes() []*resource.ListOptions {
	items := []*resource.ListOptions{}
	items = append(items, &resource.ListOptions{
