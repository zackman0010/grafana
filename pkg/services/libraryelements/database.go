...
func (l *LibraryElementService) getAllLibraryElements(c context.Context, signedInUser identity.Requester, query model.SearchLibraryElementsQuery) (model.LibraryElementSearchResult, error) {
	...
	err = l.SQLStore.WithDbSession(c, func(session *db.Session) error {
		...
		if folderFilter.includeGeneralFolder {
			...
			builder.Write(` WHERE le.org_id=?  AND le.folder_id=0 OR (le.folder_uid = "" OR le.folder_uid="General" OR le.folder_uid IS NULL)`, signedInUser.GetOrgID())
		...
		writeExcludeSQL(query, &builder)
		writeTypeFilterSQL(typeFilter, &builder)
		if err := folderFilter.writeFolderFilterSQL(folderFilter.includeGeneralFolder, &builder); err != nil {
			return err
		}
		...
		for _, element := range elements {
			...
			if query.FolderFilter == "0" && (element.FolderName != dashboards.RootFolderName) {
				continue
			}
			counts++
		}
		...
		// fmt.Println("COUNT QUERY", countBuilder.GetSQLString())
		// fmt.Println("COUNT PARAMS", countBuilder.GetParams())
		...
		fmt.Println("das", int64(len(elements)))
