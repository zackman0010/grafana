...
func (dr *DashboardServiceImpl) cleanupK8sDashboardResources(ctx context.Context, batchSize int64, timeout time.Duration) error {
	...
	for _, org := range orgs {
		...
		for {
			...
			// Find the maximum resource version from the items in this batch
			var maxBatchResourceVersion int64
			for _, item := range data.Items {
				if meta, ok := item.Object["metadata"].(map[string]interface{}); ok {
					if rv, ok := meta["resourceVersion"].(string); ok {
						if itemRV, err := strconv.ParseInt(rv, 10, 64); err == nil {
							if itemRV > maxBatchResourceVersion {
								maxBatchResourceVersion = itemRV
							}
						}
					}
				}
				...
				if meta, ok := item.Object["metadata"].(map[string]interface{}); ok {
					if dt, ok := meta["deletionTimestamp"].(string); ok {
			...
			// Update resource version after each batch if we processed any items
			if maxBatchResourceVersion > 0 {
				maxBatchResourceVersionStr := strconv.FormatInt(maxBatchResourceVersion, 10)
				if lastResourceVersion != maxBatchResourceVersionStr {
					dr.log.Info("Updating resource version after batch", "orgID", orgID, "newResourceVersion", maxBatchResourceVersionStr, "oldResourceVersion", lastResourceVersion)
					err = dr.kvstore.Set(orgCleanupCtx, orgID, k8sDashboardKvNamespace, k8sDashboardKvLastResourceVersionKey, maxBatchResourceVersionStr)
					if err != nil {
						errs = append(errs, fmt.Errorf("org %d: failed to update resource version: %w", orgID, err))
					}
					lastResourceVersion = maxBatchResourceVersionStr
				}
			}

			// Check if we have more items to process
			continueToken = ""
			if meta, ok := data.Object["metadata"].(map[string]interface{}); ok {
				if continueValue, exists := meta["continue"]; exists && continueValue != nil {
					if contStr, ok := continueValue.(string); ok && contStr != "" {
						continueToken = contStr
					}
				}
			}
